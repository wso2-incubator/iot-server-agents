...
import org.wso2.carbon.iot.android.sense.transport.TransportHandler;
...
public abstract class MQTTTransportHandler
    ...
    private void initSubscriber() {
        try {
            ...
            Log.i("MQTTTransportHandler", "MQTT subscriber was created with ClientID : " + clientId);
            ...
            String errorMsg = "MQTT Client Error\n" + "\tReason:  " + ex.getReasonCode() +
                    ...
                    "\n\tException: " + ex;
            Log.e("MQTTTransportHandler", errorMsg);
    ...
    protected void connectToQueue() throws TransportHandlerException {
        try {
            client.connect(options);
                ...
                Log.d("MQTTTransportHandler", "Subscriber connected to queue at: " + this.mqttBrokerEndPoint);
            ...
            //TODO:: Compulsory log of errors and remove formatted error
                Log.d("MQTTTransportHandler", errorMsg);
    ...
    protected void subscribeToQueue() throws TransportHandlerException {
        try {
            ...
            client.subscribe(subscribeTopic, 0);
                Log.d("MQTTTransportHandler", "Subscriber '" + clientId + "' subscribed to topic: " + subscribeTopic);
        } catch (MqttException ex) {
            //TODO:: Compulsory log of errors and remove formatted error
    ...
    protected void publishToQueue(String topic, String payLoad, int qos, boolean retained)
        ...
        try {
            client.publish(topic, payLoad.getBytes(StandardCharsets.UTF_8), qos, retained);
                Log.d("MQTTTransportHandler", "Message: " + payLoad + " to MQTT topic [" + topic +
            ...
            String errorMsg =
                    "MQTT Client Error" + "\n\tReason:  " + ex.getReasonCode() + "\n\tMessage: " +
                            ex.getMessage() + "\n\tLocalMsg: " + ex.getLocalizedMessage() +
                            "\n\tCause: " + ex.getCause() + "\n\tException: " + ex;
            Log.i("MQTTTransportHandler", errorMsg);
            throw new TransportHandlerException(errorMsg, ex);
        }
    }
    ...
    protected void publishToQueue(String topic, MqttMessage message)
        ...
        try {
            client.publish(topic, message);
                Log.d("MQTTTransportHandler", "Message: " + message.toString() + " to MQTT topic [" + topic +
                        "] published successfully");
        } catch (MqttException ex) {
            //TODO:: Compulsory log of errors and remove formatted error
            String errorMsg =
    ...
    public void connectionLost(Throwable throwable) {
           Log.w("MQTTTransportHandler", "Lost Connection for client: " + this.clientId + " to " + this
                   .mqttBrokerEndPoint + "." +
    ...
    public void messageArrived(final String topic, final MqttMessage mqttMessage) {
            Log.d("MQTTTransportHandler", "Got an MQTT message '" + mqttMessage.toString() + "' for topic '" + topic +
                    "'.");
        ...
        Thread messageProcessorThread = new Thread() {
            public void run() {
                try {
                    ...
                    Log.e("MQTTTransportHandler", "An error occurred when trying to process received MQTT message [" +
                            mqttMessage + "] " +
                            "for topic [" + topic + "].", e);
                }
            }
        ...
        try {
            if (iMqttDeliveryToken.isComplete()) {
                if (iMqttDeliveryToken.getMessage() != null) {
                    ...
                    Log.d("MQTTTransportHandler", "Message to client [" + client + "] under topic (" + topic +
                ...
                Log.w("MQTTTransportHandler", "FAILED: Delivery of MQTT message to [" + client + "] under topic [" +
                        topic + "] failed.");
            }
        } catch (MqttException e) {
            //TODO:: Throw errors
            Log.w("MQTTTransportHandler", "Error occurred whilst trying to read the message from the MQTT delivery " +
                    "token.");
        }
    }
